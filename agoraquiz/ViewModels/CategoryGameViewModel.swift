...
class CategoryGameViewModel: ObservableObject {
    @Published var currentQuestionIndex = 1
    @Published var totalQuestions = 9 // Número total de preguntas en la categoría
    ...
    func quitGame() {
        ...
        gameService.abandonGame(gameId: game.id)
            .sink(receiveCompletion: { completion in
                ...
                DispatchQueue.main.async {
                    self?.navigationManager.navigateToHome()
                }
            })
            .store(in: &cancellables)
    }
